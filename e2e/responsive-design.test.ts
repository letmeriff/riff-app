...
import { loginUser, createTestCanvas, createNode } from './utils/multi-browser-test';
...
function getViewport(testInfo) {
  ...
  const viewportAnnotation = testInfo.annotations.find(a => a.type === 'viewport');
...
test.describe('Responsive Design Testing', () => {
  ...
  test('dashboard adapts to mobile viewport', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="create-canvas-button"]')).toBeVisible();
  });
  ...
  test('canvas interaction works on mobile viewport', async ({ page }) => {
    ...
    const nodeSelector = '[data-testid="canvas-node"]:has-text("Mobile test node")';
  ...
  for (const viewport of ['mobile', 'tablet', 'laptop', 'desktop']) {
    test(`canvas renders correctly on ${viewport}`, async ({ page }) => {
      ...
      for (let i = 1; i <= 2; i++) {
        const node = page.locator(`[data-testid="canvas-node"]:has-text("Node ${i}")`);
  ...
  test('chat interface adapts to different viewports', async ({ page, browserName }) => {
    ...
    for (const viewport of ['mobile', 'tablet', 'desktop']) {
      ...
      console.log(`Testing chat interface on ${viewport}: ${JSON.stringify(viewportSizes[viewport])}`);
      ...
      const chatNode = page.locator('[data-testid="canvas-node"]:has-text("Chat Node")');
