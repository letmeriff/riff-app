...
import { test, loginUser, createTestCanvas, createNode, waitForSync } from './utils/multi-browser-test';
...
test.describe('Network Conditions Testing', () => {
  ...
  test('users can continue editing during network interruptions', async ({ userPages, browser }) => {
    // Login both users
    ...
    const offlineIndicator = userPages.bob.locator('[data-testid="offline-indicator"]');
    ...
    await createNode(userPages.alice, { x: 200, y: 400 }, aliceSecondNodeContent);
  ...
  test('reconnection after prolonged disconnection works correctly', async ({ userPages }) => {
    // Login both users
    await loginUser(userPages.alice, testUsers.alice.email, testUsers.alice.password);
    await loginUser(userPages.bob, testUsers.bob.email, testUsers.bob.password);
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      const node = bobNewPage.locator(`[data-testid="canvas-node"]:has-text("${nodeContent}")`);
  ...
  test.describe('Cross-browser Compatibility', () => {
    ...
    test('canvas renders correctly across different browsers', async ({ userPages, browserName }) => {
      ...
      for (let i = 1; i <= 3; i++) {
        const node = userPages.alice.locator(`[data-testid="canvas-node"]:has-text("Node ${i}")`);
        await expect(node).toBeVisible();
      }
    ...
    test('collaborative editing works across different browsers', async ({ userPages, browserName }) => {
...
test.describe('Throttled Network Testing', () => {
  test.skip('app functions with slow 3G connection', async ({ browser }) => {
    ...
    await expect(page.locator('[data-testid="canvas-container"]')).toBeVisible({ timeout: 30000 });
    
    // Create a node
    await page.mouse.dblclick(200, 200);
    await page.waitForSelector('[data-testid="node-editor"]', { timeout: 30000 });
    ...
    const nodeSelector = '[data-testid="canvas-node"]:has-text("Slow connection test")';
