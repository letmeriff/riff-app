...
import { test, loginUser, createTestCanvas, createNode, joinSameCanvas, waitForSync } from './utils/multi-browser-test';
...
test.describe('Collaborative Feature Testing', () => {
  ...
  test('two users can edit the same canvas simultaneously', async ({ userPages }) => {
    // Login both users
    await loginUser(userPages.alice, testUsers.alice.email, testUsers.alice.password);
    ...
    const aliceNode = await createNode(userPages.alice, aliceNodePosition, aliceNodeContent);
    
    // Wait for synchronization
    await waitForSync([userPages.alice, userPages.bob]);
    
    // Verify that Bob can see Alice's node
    const bobsViewOfAliceNode = userPages.bob.locator(`[data-testid="canvas-node"]:has-text("${aliceNodeContent}")`);
    await expect(bobsViewOfAliceNode).toBeVisible();
    
    // Bob creates a node
    const bobNodePosition = { x: 400, y: 200 };
    const bobNodeContent = 'Node created by Bob';
    const bobNode = await createNode(userPages.bob, bobNodePosition, bobNodeContent);
    
    // Wait for synchronization
    await waitForSync([userPages.alice, userPages.bob]);
    
    // Verify that Alice can see Bob's node
    const alicesViewOfBobNode = userPages.alice.locator(`[data-testid="canvas-node"]:has-text("${bobNodeContent}")`);
    await expect(alicesViewOfBobNode).toBeVisible();
  });

  test('changes persist when a user disconnects and reconnects', async ({ userPages }) => {
    // Login both users
    await loginUser(userPages.alice, testUsers.alice.email, testUsers.alice.password);
    await loginUser(userPages.bob, testUsers.bob.email, testUsers.bob.password);
    ...
    const bobsViewOfNode = bobNewPage.locator(`[data-testid="canvas-node"]:has-text("${nodeContent}")`);
    await expect(bobsViewOfNode).toBeVisible();
  });

  test('user awareness indicators show which users are online', async ({ userPages }) => {
    // Login both users
    await loginUser(userPages.alice, testUsers.alice.email, testUsers.alice.password);
    await loginUser(userPages.bob, testUsers.bob.email, testUsers.bob.password);
    ...
    const alicesViewOfUserList = userPages.alice.locator('[data-testid="user-presence-list"]');
    await expect(alicesViewOfUserList).toContainText('Bob');
    
    // Verify that Bob can see Alice is online
    const bobsViewOfUserList = userPages.bob.locator('[data-testid="user-presence-list"]');
  ...
  test('user cursor positions are synchronized', async ({ userPages }) => {
    ...
    const alicesViewOfBobsCursor = userPages.alice.locator('[data-testid="remote-cursor"][data-user-id="bob"]');
  ...
  test('concurrent edits on the same node are resolved properly', async ({ userPages }) => {
    // Login both users
    await loginUser(userPages.alice, testUsers.alice.email, testUsers.alice.password);
    await loginUser(userPages.bob, testUsers.bob.email, testUsers.bob.password);
    ...
    const node = await createNode(userPages.alice, { x: 200, y: 200 }, initialContent);
    ...
    await node.dblclick();
    const bobsNode = userPages.bob.locator(`[data-testid="canvas-node"]:has-text("${initialContent}")`);
    await bobsNode.dblclick();
    
    // Alice edits the node
    await userPages.alice.fill('[data-testid="node-content"]', 'Alice edited this node');
    await userPages.alice.click('[data-testid="save-node-button"]');
    
    // Bob edits the same node with different content
    await userPages.bob.fill('[data-testid="node-content"]', 'Bob edited this node');
    ...
    const alicesNodeAfterEdit = userPages.alice.locator('[data-testid="canvas-node"]').first();
    const bobsNodeAfterEdit = userPages.bob.locator('[data-testid="canvas-node"]').first();
